...
\midrule
CHAN, Hok Nam    & Travel time matrix,  discrete-event simulation harness, data analysis, NLI layer& 45\%& 50\%\\[4pt]
CHOI, Cheuk Man & LLM integration, Flask web API, frontend dashboard, OR-Tools solver design, policy advisor(RAG)& 45\%& 50\%\\[4pt]
KIM, Youseung   & Communication with external parties& 10\%& 0\%\\
\bottomrule
\end{tabular}
\end{center}
...
HHH's stated Key Performance Indicator (KPI) requires a porter to arrive at the pickup location within 15 minutes of an order being placed. This response-time metric is operationally meaningful: it captures the component of the task that dispatch decisions directly control --- how quickly the right porter reaches the patient or specimen. Analysis of 81,349 historical records from 2024 reveals a 41.3\% pickup-KPI violation rate under current operations, with a long-tailed pickup-time distribution (median 12.0 min, mean 35.6 min). This pattern --- where the median task already meets the target but tail latency drags the mean far above it --- motivates an automated, optimised dispatch system focused on tail compression.
...
\node[box=blue]            (nlp)    at (0,0)     {Layer 1: NLI\\[1pt]\footnotesize DeepSeek LLM};
\node[box=green!60!black]  (solver) at (6.0,0)   {Layer 2: Solver\\[1pt]\footnotesize OR-Tools VRPTW};
\node[box=orange]          (ui)     at (12.0,0)  {Layer 3: Interface\\[1pt]\footnotesize Flask + JS};

% Bottom row with extra vertical clearance
\node[box=gray]            (data)   at (6.0,-3.0)  {Data Layer\\[1pt]\footnotesize 89$\times$89 Matrix};
\node[box=violet]          (adv)    at (12.0,-3.0) {Layer 4: Policy Advisor\\[1pt]\footnotesize RAG + LLM};
...
\draw[arr] (adv) -- (ui);
\end{tikzpicture}
\caption{Four-layer hybrid AI dispatch system architecture. Input flows left-to-right through the NLI (Layer 1), Solver (Layer 2), and Interface (Layer 3) layers; the Data Layer feeds both the Solver and the Policy Advisor (Layer 4).}
\label{fig:architecture}
\end{figure}

\textbf{Layer 1 (NLI)}: Converts free-text dispatch requests into structured task JSON using a DeepSeek LLM. \textbf{Layer 2 (Solver)}: Assigns tasks to porters using OR-Tools VRPTW with rolling-horizon re-optimisation. \textbf{Layer 3 (Interface)}: Flask REST API and Vanilla JS frontend dashboard. \textbf{Layer 4 (Policy Advisor)}: RAG-based module answering operational Q\&A and assessing KPI risk.

\subsection{Data Source}

The \path{DATA2024.xlsx} dataset used throughout this work was assembled by merging twelve monthly task extracts supplied by HHH for the 2024 operating year. After concatenation, records with status \begin{CJK}{UTF8}{min}已傳真\end{CJK} (faxed but not subsequently dispatched through the porter system) were excluded, leaving 81,349 completed task records for analysis. This single dataset is the common substrate for three downstream uses: (i) calibrating the 89$\times$89 travel-time matrix consumed by the Solver (\S\ref{sec:travel}); (ii) populating the Policy Advisor's retrieval index; and (iii) providing the historical baseline against which the simulation results in \S\ref{sec:baseline} are measured.
...
Each dispatch decision is formalised as a small instance of the Vehicle Routing Problem with Time Windows (VRPTW). Informally: given the porters available right now (each at a known location) and the tasks waiting to be served, the solver chooses an assignment that minimises total porter travel time while penalising any task whose pickup response exceeds 15 minutes. The formal notation follows.

Let $P = \{p_1, \ldots, p_m\}$ be the set of available porters and $T = \{t_1, \ldots, t_n\}$ be the pending tasks. Each task $t_k$ has origin $o_k$, destination $d_k$, and service time $s_k$. The service time $s_k$ is a per-service-type constant calibrated from observed historical means (e.g., 5 min for specimen transport, 7 min for patient return, 10 min for patient transport), capturing the time a porter spends at the origin handling the task before departing for the destination. Porter $p_i$ is at known location $l_i$. The KPI soft time window is $[0, W]$ where $W = 15$ minutes.
...
\subsection{Layer 4: Policy Advisor (RAG Design)}
...
\textbf{Knowledge base (HistoricalTaskStore):} The DATA2024.xlsx dataset is loaded into memory at startup, providing all 81,349 cleaned task records for retrieval. (The narrower 80,078-record baseline cohort reported in \S\ref{sec:baseline} excludes a further 1,271 rows whose elapsed times exceeded 24 hours; those rows have valid origin, destination, service, and arrival information and are retained here, since the duration anomaly affects only summary statistics and not the retrieval features.) Each record represents one completed porter task with fields including service type, origin, destination, time of day, and total duration.
...
\textbf{Replay mode:} 100 tasks from DATA2024.xlsx are replayed in chronological order, preserving real inter-arrival times. This tests the solver under the actual task distribution and volumes observed at HHH. The 100-task slice was chosen to approximate a single peak-shift workload while keeping each simulation run computationally tractable; extending the replay to a full-day or full-week horizon is straightforward future work.
...
Three metrics are tracked: (1) \textbf{total task duration} (order placement to delivery completion); (2) \textbf{time-to-pickup} (order placement to porter arrival at origin --- the KPI-relevant metric); (3) \textbf{KPI violation rate} against both metrics.

A simulation run is described as \emph{queue-saturated} when the task arrival rate exceeds the maximum task-completion rate of the available fleet, so the queue grows unboundedly over the run. In this regime, dispatch optimisation can only marginally affect outcomes because tasks wait primarily for porter availability, not for the assignment decision. This terminology recurs in \S\ref{sec:simresults} when interpreting why replay-mode improvements are smaller than synthetic-mode improvements.
...
\textbf{Priority extraction errors (12 of 100):} All 12 errors follow the same pattern: the ground-truth label is ``Urgent'' (corresponding to the Chinese term \begin{CJK}{UTF8}{min}即時\end{CJK}, meaning ``immediate'') but the LLM returned ``Normal''. No errors occurred in the ``Super-Urgent'' category. The root cause is semantic ambiguity: in the reconstructed test sentences, Urgent tasks are represented by the term ``Immediate'' in the NL sentence, which does not carry an explicit superlative marker. The LLM interprets ``Immediate X-ray from 4D to Radiology'' as a normal-priority request because the urgency modifier is conveyed by word choice rather than an explicit flag. Table~\ref{tab:nlp_errors} shows representative error cases.
...
\textbf{Proposed fix:} Adding an explicit disambiguation rule to the system prompt --- ``If the request contains the word `Immediate' or `\begin{CJK}{UTF8}{min}即時\end{CJK}' without a `Super-Urgent' marker, classify priority as Urgent'' --- would close this gap. This is a one-line prompt change, not a model retraining requirement.
...
Analysis of the historical task records from DATA2024.xlsx reveals the pre-intervention operational baseline. Of the 81,349 cleaned records, 1,271 were excluded from duration statistics because their elapsed times exceeded 24 hours --- a pattern indicating delayed completion timestamps (porters marking tasks complete in a later shift) rather than genuine multi-day journeys. The reported statistics are therefore based on $n = 80{,}078$ records with both \begin{CJK}{UTF8}{min}下單\end{CJK} (order) and \begin{CJK}{UTF8}{min}報到\end{CJK} (porter arrival) timestamps and a plausible duration.

Two complementary metrics are reported. \textit{Pickup time} (\begin{CJK}{UTF8}{min}報到\end{CJK} $-$ \begin{CJK}{UTF8}{min}下單\end{CJK}) is the order-to-arrival interval that the 15-minute KPI directly targets. \textit{Total duration} (\begin{CJK}{UTF8}{min}完成\end{CJK} $-$ \begin{CJK}{UTF8}{min}下單\end{CJK}) additionally includes service time at origin and travel to destination, which dispatch decisions cannot influence.

\begin{itemize}
  \item Mean pickup time: \textbf{35.6 minutes}; median pickup time: \textbf{12.0 minutes}
  \item Pickup-KPI violation rate ($>15$ min): \textbf{41.3\%}
  \item Mean total task duration: \textbf{46.3 minutes}; median: \textbf{24.2 minutes}
\end{itemize}

The 35.6 min mean against a 12.0 min median signals a right-skewed pickup distribution: the median task is already inside the KPI, but a long tail of delayed pickups pushes the mean far above target. This is the regime in which dispatch optimisation is most impactful --- compressing the tail rather than improving the median. Table~\ref{tab:by_service} summarises the major service types.

\begin{table}[H]
\centering
\caption{Historical pickup-KPI performance by major service type (n\,=\,80{,}078). Pickup Violation reports the proportion of tasks where porter arrival exceeded 15 minutes; Mean Total Duration is shown for context but is not the KPI metric.}
\label{tab:by_service}
\begin{tabular}{lccc}
\toprule
\textbf{Service Type} & \textbf{Count} & \textbf{Mean Total Duration (min)} & \textbf{Pickup Violation (\%)} \\
\midrule
Patient Return          & 13{,}660  & 20.6  & 35.1\% \\
Document Delivery       & 10{,}059  & 47.6  & 40.0\% \\
X-ray Transport         &  8{,}398  & 54.3  & 68.8\% \\
Admission               & 10{,}368  & 32.5  & 25.3\% \\
Specimen Transport      &  6{,}452  & 17.9  & 19.3\% \\
NEATS Patient           &  8{,}330  & 25.0  & 35.8\% \\
3F X-ray                &  4{,}891  & 177.0 & 88.2\% \\
Patient Transport       &  2{,}537  & 69.3  & 44.8\% \\
\bottomrule
\end{tabular}
\end{table}

The pickup-violation profile differs sharply from the total-duration profile. Specimen transport (19.3\% pickup violation) and admissions (25.3\%) already meet the pickup KPI for the majority of historical tasks, despite admissions in particular having a longer total duration (32.5 min) once travel-to-destination is included. By contrast, 3F X-ray (88.2\%) and X-ray Transport (68.8\%) struggle on pickup --- typically because the imaging suites are concentrated on specific floors, leaving few porters geographically close to the pickup origin. Notably, \textbf{57.0\%} of all unique origin--destination routes in the dataset have direct travel plus minimum service time exceeding 15 minutes, meaning no dispatch algorithm can achieve full task \textit{completion} within 15 minutes for these routes. This is precisely why the pickup-time KPI (which dispatch directly controls) and not the total-duration KPI (which is geographically bounded) is the appropriate optimisation target.
...
\begin{table}[H]
\centering
\caption{Simulation results: mean total task duration by strategy, mode, and fleet size. Total duration is reported for completeness; the pickup-time KPI metric is reported separately in Table~\ref{tab:pickup}.}
...
The divergence between these two regimes is deliberate rather than artifactual. The 100-task replay slice compresses real timestamps into a load profile that exceeds steady-state fleet capacity, stress-testing system behaviour at peak overload. Synthetic mode, by contrast, draws inter-arrivals from a Poisson process at the 8-minute mean that matches HHH's actual operational rate of approximately 10 tasks per hour per shift --- the regime in which a deployed system would spend the majority of its runtime. The 17--30\% improvements observed in synthetic mode therefore represent the practically realisable gains under normal operations, while the replay results characterise robustness under peak congestion. Both regimes are operationally relevant: synthetic mode informs day-to-day deployment decisions, and replay mode bounds worst-case performance.
...
\begin{enumerate}
  ...
  \item \textbf{The 15-minute KPI is achievable with appropriate fleet sizing.} At 15 porters, the mean pickup time is \textbf{15.9 minutes} --- essentially at the KPI target. This demonstrates that the 15-minute response KPI is physically achievable, unlike the total-duration KPI (which is blocked by geography for 57.0\% of routes). The implication for HHH management is that fleet expansion to 15+ porters, combined with OR-Tools dispatch, would bring mean response times into KPI compliance.
\end{enumerate}
...
\subsection{End-to-End Functional Validation}
\label{sec:edgecases}

In addition to the aggregate metrics reported above, the deployed prototype was exercised against a set of representative edge cases that span the system's defined operational scope. Each case was issued through the live web dashboard and the resulting behaviour observed end-to-end. Table~\ref{tab:edgecases} summarises the cases and the system's response.

\begin{table}[H]
\centering
\caption{End-to-end functional validation across representative dispatch scenarios.}
\label{tab:edgecases}
\begin{tabular}{p{4.4cm}p{5.0cm}p{4.6cm}}
\toprule
\textbf{Scenario} & \textbf{Input (NL request)} & \textbf{Observed system behaviour} \\
\midrule
Multi-task utterance &
``Send a patient from 7H to A\&D and a specimen from 5L to Lab'' &
NLI returns two structured tasks; solver assigns them concurrently to two porters in a single dispatch call. \\[4pt]
Future-scheduled task &
``Send a patient from 7H to A\&D at 14:30'' &
NLI extracts \texttt{scheduled\_at}; task placed on \texttt{scheduled\_queue}; daemon thread releases it at 14:30; UI shows live countdown. \\[4pt]
Infection control flag &
``NEATS patient from 3C to A\&D, VRE positive'' &
NLI populates \texttt{infection\_control}; assignment card surfaces a red infection warning to the dispatcher. \\[4pt]
Mixed Chinese / English input &
``\begin{CJK}{UTF8}{min}緊急\end{CJK} patient from 3FXRAY to 6H'' &
NLI parses the Chinese urgency token and the English route; priority correctly classified as Urgent. \\[4pt]
Invalid origin location &
``Send a patient from FloorZ to A\&D'' &
Travel-matrix validator rejects \texttt{FloorZ}; the dispatcher receives a structured error message naming the unknown location and listing the 89 valid options. \\[4pt]
Solver failure / fallback &
(OR-Tools unavailable at runtime) &
System falls back to the greedy heuristic; the assignment still completes, with a notice to the dispatcher that optimisation is degraded. \\[4pt]
Multi-stop route &
``Send a patient from 7H to 3FXRAY, then continue to ICU'' &
NLI extracts \texttt{from=7H}, \texttt{stops=["3FXRAY"]}, \texttt{to=ICU}; the solver routes the porter through the intermediate stop before the final destination, maintaining the pickup-KPI constraint at each leg. \\
\bottomrule
\end{tabular}
\end{table}

These scenarios cover every public dispatch path documented in Appendix~A (\texttt{/dispatch}, \texttt{/scheduled}, \texttt{/advisor/*}) as well as the two principal failure modes (invalid input, solver unavailability). The system handled each case correctly without manual intervention. Combined with the simulation results (\S\ref{sec:simresults}--\S\ref{sec:pickup}), this functional sweep establishes that the prototype works for the most common real cases within its defined scope rather than only for the metric-aggregation regime.
...
\begin{enumerate}
  ...
  \item \textbf{Operational intelligence via RAG:} A policy advisor grounding LLM-generated recommendations in 81,349 historical task records, providing KPI risk assessments, data-driven fleet suggestions, and free-text Q\&A without bespoke model training.
