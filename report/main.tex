...
This report documents the design, implementation, and evaluation of an AI-driven porter dispatch system developed for HHH as a two-semester Final Year Project in Industrial Engineering and Decision Analytics. The project spans natural language processing, vehicle routing optimisation, real-time web systems, and data-driven policy analysis.
...
Hospital porter services are a critical component of clinical workflow, yet in many institutions dispatch remains a manual, radio-based process that routinely fails to meet operational targets. At HHH, porters are tasked with transporting patients, specimens, and equipment between 89 hospital departments. The stated Key Performance Indicator (KPI) requires a porter to arrive at the pickup location within 15 minutes of an order being placed. Analysis of 2,843 historical records from 2024 reveals that this target is violated in the majority of cases, with mean pickup response times far exceeding 15 minutes under current fleet configurations.
...
HHH's stated Key Performance Indicator (KPI) requires a porter to arrive at the pickup location within 15 minutes of an order being placed. This response-time metric is operationally meaningful: it captures the component of the task that dispatch decisions directly control --- how quickly the right porter reaches the patient or specimen. Analysis of 2,843 historical records from 2024 reveals that this KPI is violated in the majority of recorded tasks under current operations, motivating the development of an automated, optimised dispatch system.
...
\node[box=blue]   (nlp)    at (0,0)   {NLI Layer\\[-2pt]\small DeepSeek LLM};
\node[box=green!70!black]  (solver) at (5.2,0)   {Solver Layer\\[-2pt]\small OR-Tools VRPTW};
\node[box=orange] (ui)     at (10.4,0) {Interface Layer\\[-2pt]\small Flask + JS};
\node[box=gray]   (data)   at (5.2,-2.4) {Data Layer\\[-2pt]\small 89$\times$89 Matrix\\[-4pt]\small DATA2024.xlsx};
\node[box=violet] (adv)    at (10.4,-2.4) {Policy Advisor\\[-2pt]\small RAG + LLM};

\node[lbl, left=0.5cm of nlp]   (in)  {\shortstack{Natural language\\request (ZH/EN)}};
\node[lbl, right=0.5cm of ui]   (out) {\shortstack{Real-time\\dashboard}};

\draw[arr] (in)  -- (nlp);
\draw[arr] (nlp) -- node[above,lbl]{Structured JSON} (solver);
\draw[arr] (solver) -- node[above,lbl]{\shortstack{Assignment +\\explanation}} (ui);
\draw[arr] (ui)  -- (out);
...
\draw[arr] (adv) -- (ui);
\end{tikzpicture}
\caption{Four-layer hybrid AI dispatch system architecture.}
...
\begin{equation}
\text{sim}(q, r) = w_s \cdot \mathbf{1}[\text{service match}] + w_o \cdot \mathbf{1}[\text{origin match}] + w_d \cdot \mathbf{1}[\text{dest match}] + w_t \cdot \mathbf{1}[\text{time-of-day match}]
\end{equation}
with weights $w_s = 0.4$, $w_o = 0.3$, $w_d = 0.2$, $w_t = 0.1$, reflecting the relative predictive importance of each field for task duration.
...
Analysis of 2,843 historical tasks from DATA2024.xlsx reveals the pre-intervention operational baseline:
...
\node[font=\small\bfseries] at (5.5, 5.8) {OR-Tools Improvement Over Greedy in Mean Total Duration (\%)};
\node[font=\small] at (1.7, 5.2) {\textbf{Replay Mode}};
\fill[blue!40] (0.2,0) rectangle (1.2, 2.3) node[above,font=\scriptsize]{2.3\%};
\fill[blue!40] (1.4,0) rectangle (2.4, 2.2) node[above,font=\scriptsize]{2.2\%};
\fill[blue!40] (2.6,0) rectangle (3.6, 0.9) node[above,font=\scriptsize]{0.9\%};
\node[font=\scriptsize] at (0.7,  -0.3) {3};
\node[font=\scriptsize] at (1.9,  -0.3) {5};
\node[font=\scriptsize] at (3.1,  -0.3) {7};
\node[font=\scriptsize] at (1.9,  -0.7) {Fleet size (porters)};
\node[font=\small] at (7.5, 5.2) {\textbf{Synthetic Mode}};
\fill[green!50!black!60] (4.8,0) rectangle (5.8, 1.02) node[above,font=\scriptsize]{10.2\%};
\fill[green!50!black!60] (6.0,0) rectangle (7.0, 3.05) node[above,font=\scriptsize]{30.5\%};
\fill[green!50!black!60] (7.2,0) rectangle (8.2, 1.69) node[above,font=\scriptsize]{16.9\%};
\node[font=\scriptsize] at (5.3,  -0.3) {3};
\node[font=\scriptsize] at (6.5,  -0.3) {5};
\node[font=\scriptsize] at (7.7,  -0.3) {7};
\node[font=\scriptsize] at (6.5,  -0.7) {Fleet size (porters)};
\draw[->] (0,0) -- (0,5.5) node[left,font=\scriptsize]{Improvement (\%)};
\foreach \y in {0,1,2,3,4,5}
  \draw (-0.1,\y) -- (0.1,\y) node[left,font=\scriptsize]{\y0\%};
\node[font=\scriptsize,left] at (-0.15, 0) {0\%};
\end{scope}
\end{tikzpicture}
\caption{Percentage improvement of OR-Tools over greedy in mean total task duration, by fleet size and simulation mode. Synthetic mode shows substantially larger gains because the system is not queue-saturated.}
