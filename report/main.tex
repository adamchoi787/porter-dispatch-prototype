...
HHH's stated Key Performance Indicator (KPI) requires a porter to arrive at the pickup location within 15 minutes of an order being placed. This response-time metric is operationally meaningful: it captures the component of the task that dispatch decisions directly control --- how quickly the right porter reaches the patient or specimen. Analysis of 81,349 historical records from 2024 reveals that this KPI is violated in the majority of recorded tasks under current operations, motivating the development of an automated, optimised dispatch system.
...
From the 81,349 historical tasks in DATA2024.xlsx, 80,077 valid tasks remain after removing erroneous records. Analysis of this cleaned dataset reveals the pre‑intervention operational baseline:

\begin{itemize}
  \item Mean task duration: \textbf{46.3 minutes}
  \item Median task duration: \textbf{24.2 minutes}
  \item KPI violation rate (15-min pickup threshold): \textbf{75.7\%}
\end{itemize}

The mean-median gap (46.3 vs 24.2 min) indicates a right-skewed distribution driven by high-duration service types. Table~\ref{tab:by_service} summarises the major service types by KPI performance.

\begin{table}[H]
\centering
\caption{Historical KPI violation rate by major service type (n\,=\,80{,}077).}
\label{tab:by_service}
\begin{tabular}{lccc}
\toprule
\textbf{Service Type} & \textbf{Count} & \textbf{Mean Duration (min)} & \textbf{KPI Violation (\%)} \\
\midrule
Patient Return          & 13{,}660  & 20.6  & 63.2\% \\
Document Delivery       & 10{,}059  & 47.6  & 71.1\% \\
X-ray Transport         &  8{,}398  & 54.3  & 80.1\% \\
Admission               & 10{,}368  & 32.5  & 95.8\% \\
Specimen Transport      &  6{,}452  & 17.9  & 50.5\% \\
NEATS Patient           &  8{,}330  & 25.0  & 86.6\% \\
3F X-ray                &  4{,}891  & 177.0 & 93.4\% \\
Patient Transport       &  2{,}537  & 69.3  & 80.6\% \\
\bottomrule
\end{tabular}
\end{table}

Specimen transport is the only service type approaching KPI feasibility (50.5\% violation rate, mean 17.9 min), consistent with its typically short travel distances. The high violation rates for admissions (95.8\%) and 3F X-ray (93.4\%) reflect long fixed travel distances intrinsic to the hospital geography. Notably, \textbf{57.0\%} of all unique origin--destination routes in the dataset have a direct travel time plus minimum service time exceeding 15 minutes --- meaning no dispatch algorithm can achieve task \textit{completion} within 15 minutes for these routes. This distinguishes the total-duration KPI (physically unachievable for many tasks) from the pickup-response KPI (achievable with appropriate fleet sizing and routing).
...
\begin{tabular}{llcc}
\toprule
\textbf{Mode} & \textbf{Strategy} & \textbf{Porters} & \textbf{Mean Duration (min)} \\
\midrule
