...
\begin{tikzpicture}
  ...
  %% Labels
  \node[font=\scriptsize] at (2.5,-0.3) {2,843 historical tasks, 2024};
  \node[font=\small\bfseries, color=alertred] at (2.5,-0.75)
    {Mean duration: \textbf{49.7 min}};
\end{tikzpicture}
\end{columns}
\end{frame}
...
\textbf{By service type (violation rate):}
...
\begin{axis}[
  ...
  xlabel={KPI Violation Rate (\%)},
  xmin=0, xmax=105,
  symbolic y coords={
    {Specimen},{Patient Return},{NEATS},{Document},{X-ray},{Patient Transpt},{3F X-ray},{Admission}},
  ytick=data,
...
\begin{block}{Key insight}
  High violation rates for admission (95.4\%) and 3F X-ray (94.4\%) reflect \textbf{long fixed travel distances} --- not dispatcher error.
\end{block}
...
\vspace{0.3cm}
\textbf{Priority: 88\%}\\
{\small All 12 errors: ``Immediate'' (\textit{ji shi}) parsed as Normal instead of Urgent. Systematic, fixable with one prompt rule.}
...
\vspace{0.2cm}
{\scriptsize\color{gray} 56.6\% of routes: full task $>$15 min regardless of algorithm (fixed by geography, not dispatch)}
...
\begin{tikzpicture}[scale=0.88]
  ...
  \node[font=\tiny] at (2.75,2.75) {KPI violation rate: 94.4\%};
  ...
  \node[font=\tiny] at (2.75,0.9) {Based on 2,843 records: 3F X-ray has};
  \node[font=\tiny] at (2.75,0.65) {mean 156.8 min and 94.4\% KPI violation.};
...
\begin{block}{3. Operational Intelligence}
  \begin{itemize}
    \item RAG policy advisor over 2,843 historical tasks
...
{\small 56.6\% of all routes have direct travel + service time $>$15 min --- KPI blocks are geographic for these tasks}
\end{frame}
...
\begin{tabular}{llccc}
\toprule
\textbf{Mode} & \textbf{Strategy} & \textbf{Porters} & \textbf{Mean (min)} & \textbf{KPI Viol.\ (\%)} \\
\midrule
